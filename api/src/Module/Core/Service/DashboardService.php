...
/**
 * Vue transverse en lecture seule : agrège les données des modules sans passer par
 * leurs API (CQRS allégé — les écritures restent dans chaque module).
 */
final class DashboardService
{
    ...
    public function overview(bool $includeActivity): array
    {
        ...
        return [
            ...
            // L'extrait du journal d'audit suit la même règle d'accès que /api/audit.
            'recentActivity' => $includeActivity ? $this->recentActivity() : [],
        ];
    }
    ...
    private function revenueByMonth(\DateTimeImmutable $today, array $invoices): array
    {
        ...
        foreach ($months as $month => $amounts) {
            $result[] = ['month' => $month, 'paid' => round($amounts['paid'], 2), 'pending' => round($amounts['pending'], 2)];
        }
    ...
    private function todos(\DateTimeImmutable $today, array $consultants, array $invoices, array $opportunities): array
    {
        ...
        $soonest = null;
        foreach ($opportunities as $opportunity) {
            if ($opportunity->getStage()->isOpen() && (null === $soonest || $opportunity->getExpectedCloseAt() < $soonest->getExpectedCloseAt())) {
                $soonest = $opportunity;
            }
        }
        if (null !== $soonest) {
