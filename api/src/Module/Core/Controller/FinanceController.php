...
/**
 * Vue direction : marge par mission et par client.
 * Coût estimé = jours ouvrés écoulés de chaque affectation × allocation × coût jour chargé.
 * C'est une approximation assumée (le réel viendrait du CRA validé) — la méthode est affichée à l'écran.
 */
final class FinanceController extends AbstractController
{
    ...
    public function margins(): JsonResponse
    {
        if (!$this->isGranted('ROLE_MANAGER') && !$this->isGranted('ROLE_ADMIN')) {
            return $this->json(['error' => 'Les marges sont réservées aux managers et à la direction.'], 403);
        }
        ...
        // CA facturé par mission (hors brouillons).
